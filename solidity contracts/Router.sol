...
library CrosswiseLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'4f03d3ed035ac8adf220b56839ebc048cb64e77c9b796d2e863816d9d93b056c' // init code hash
            ))));
    }
...
contract CrosswiseRouter is ICrosswiseRouter02, BaseRelayRecipient {
    using SafeMath for uint;
    ...
    receive() external payable {
        assert(__msgSender() == WBNB); // only accept ETH via fallback from the WBNB contract
    }
    ...
    function _addLiquidity( 
        ...
        if (reserveA == 0 && reserveB == 0) {
            lpCreators[ICrosswiseFactory(factory).getPair(tokenA, tokenB)] = __msgSender();
    ...
    function addLiquidity(
        ...
        TransferHelper.safeTransferFrom(tokenA, __msgSender(), pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, __msgSender(), pair, amountB);
    ...
    function addLiquidityETH(
        ...
        console.log("Amounts Desired: ", amountTokenDesired, msg.value);
        console.log("Amounts Required: ", amountToken, amountETH);
        address pair = CrosswiseLibrary.pairFor(factory, token, WBNB);
        TransferHelper.safeTransferFrom(token, __msgSender(), pair, amountToken);
        IWBNB(WBNB).deposit{value: amountETH}();
        assert(IWBNB(WBNB).transfer(pair, amountETH));
        liquidity = ICrosswisePair(pair).mint(to);
        // refund dust eth, if any
        if (msg.value > amountETH) TransferHelper.safeTransferETH(__msgSender(), msg.value - amountETH);
    }
    ...
    function removeLiquidity(
        ...
        address pair = CrosswiseLibrary.pairFor(factory, tokenA, tokenB);
        ICrosswisePair(pair).transferFrom(__msgSender(), pair, liquidity); // send liquidity to pair
    ...
    function removeLiquidityWithPermit(
        ...
        ICrosswisePair(pair).permit(__msgSender(), address(this), value, deadline, v, r, s);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], __msgSender(), CrosswiseLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        _swap(amounts, path, to);
        // refund dust eth, if any
        if (msg.value > amounts[0]) TransferHelper.safeTransferETH(__msgSender(), msg.value - amounts[0]);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        verifyPrice(path, amountIn, 0);
        TransferHelper.safeTransferFrom(
            path[0], __msgSender(), CrosswiseLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
