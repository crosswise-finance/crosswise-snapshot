 ...
 * via __msgSender() and msg.data, they should not be accessed in such a direct
 ...
 */
contract Context is BaseRelayRecipient {
...
contract CrssToken is BaseRelayRecipient, IBEP20, Ownable, Initializable {
    ...
    constructor(
        ...
        _excludedFromAntiWhale[__msgSender()] = true;
    ...
    function init_router(address router) public {
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        _approve(__msgSender(), spender, amount);
        return true;
    }
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        _approve(__msgSender(), spender, _allowances[__msgSender()][spender].add(addedValue));
        return true;
    }
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        _approve(
            __msgSender(),
            spender,
            _allowances[__msgSender()][spender].sub(subtractedValue, 'BEP20: decreased allowance below zero')
        );
        return true;
    }
    ...
    function transfer(address recipient, uint256 amount) public override antiWhale(__msgSender(), recipient, amount) returns (bool) {
        require(recipient != address(0), "BEP20: transfer to the zero address");
        require(balanceOf(__msgSender()) >= amount, "BEP20: transfer amount exceeds balance");

        if (whitelist[recipient] || whitelist[__msgSender()]) {
            _transfer(__msgSender(), recipient, amount);
            _moveDelegates(_delegates[__msgSender()], _delegates[recipient], amount);
            emit WhitelistedTransfer(__msgSender(), recipient, amount);
        } else {
            ...
            if (
                ...
                _transfer(__msgSender(), address(this), liquidityAmount);
                
                // AUDIT : CTC-09 | Voting Power Not Moved Along With Liquify
                _moveDelegates(_delegates[__msgSender()], _delegates[address(this)], liquidityAmount);
                swapAndLiquify();
            }
            ...
            if(recipient == crssBnbPair) {
                ...
                _transfer(__msgSender(), recipient, transferAmount);
                _moveDelegates(_delegates[__msgSender()], _delegates[recipient], transferAmount);

                _transfer(__msgSender(), devTo, devAmount);
                _moveDelegates(_delegates[__msgSender()], _delegates[devTo], devAmount);

                _transfer(__msgSender(), buybackTo, buybackAmount);
                _moveDelegates(_delegates[__msgSender()], _delegates[buybackTo], buybackAmount);
            }
        }
    ...
    function transferFrom(
        ...
        _approve(
            sender,
            __msgSender(),
            allowance(sender,__msgSender()).sub(amount, "BEP20: transfer amount exceeds allowance")
        );
        return true;
    }
    ...
    function claimV1Token(uint256 amount) external {
        // AUDIT : CTC-07 | Discussion on claimV1Token() Function
        // DEV : We get balance of __msgSender() before transferFrom because of the fees.
        // uint256 balance = oldCrss.balanceOf(__msgSender());
        // uint256 balance1 = oldCrss1.balanceOf(__msgSender());
        // uint256 mintBalance = balance.add(balance1);

        // // AUDIT : CTC-01 | Return value not handled
        // bool success = oldCrss.transferFrom(__msgSender(), burnAddress, balance);
        // require(success, "Trasnfer token Failed");
        // bool success1 = oldCrss1.transferFrom(__msgSender(), burnAddress, balance1);
        // require(success1, "Trasnfer token Failed");
        
        _mint(__msgSender(), amount);
        
        // AUDIT : CTC-06 | Does not move delegates while mint token
        _moveDelegates(address(0), _delegates[__msgSender()], amount);

        // AUDIT : CTC-03 | Missing Emit Events
        emit ClaimV1Token(__msgSender(), amount);
    }
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public  {
     ...
     function setMaxTransferAmountRate(uint256 _maxTransferAmountRate) public  {
    ...
    function setWhiteList(address _addr, bool _status) external  {
    ...
    /**
     * @dev Delegate votes from `__msgSender()` to `delegatee`
    ...
    */
    function delegate(address delegatee) external {
        return _delegate(__msgSender(), delegatee);
    }
 ...
 * This contract is only required for intermediate, library-like contracts.
 */
contract ContextUpgradeSafe is Initializable, BaseRelayRecipient {
...
contract xCrssToken is BaseRelayRecipient, BEP20UpgradeSafe {
    ...
    function depositToken(address _depositUser, uint256 _rewardAmount) public {
        require(__msgSender() == masterChef || __msgSender() == stakingVault, "xCrssToken.deposit: Sender must be masterChef or stakingVault contract");
        ...
        crssToken.transferFrom(__msgSender(), address(this), _rewardAmount);
    ...
    function withdrawToken(uint256 _amount) public {
        uint256 unlocked = unlockedToken(__msgSender());
        require(unlocked >= _amount, "xCrssToken.withdrawToken: Not enough token to withdraw.");

        withdrawByElement(__msgSender(), _amount);

        userWithdrawAmount[__msgSender()] = userWithdrawAmount[__msgSender()].add(_amount);
        
        crssToken.transfer(__msgSender(), _amount);
        
        _burn(__msgSender(), _amount);

        emit WithdrawToken(__msgSender(), _amount);
    }
...
contract MasterChef is ReentrancyGuard, BaseRelayRecipient {
    ...
    modifier onlyOwner() {
        require(owner() == __msgSender(), "caller is not the owner");
        _;
    }
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint256 _depositFeeBP, address _strategy, bool _withUpdate) external  {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint256 _depositFeeBP, address _strategy, bool _withUpdate) external validatePoolByPid(_pid)  {
    ...
    function massStakeReward(uint256[] calldata pools) external {
        uint256 oldBalance = crss.balanceOf(__msgSender());

        massHarvest(pools);

        uint256 newBalance = crss.balanceOf(__msgSender());
        uint256 amount = newBalance.sub(oldBalance);
        UserInfo storage user = userInfo[0][__msgSender()];
        if(user.amount > 0) {
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer, bool isVest, bool isAuto) public nonReentrant validatePoolByPid(_pid) {
        ...
        UserInfo storage user = userInfo[_pid][__msgSender()];
        console.log("User: ", __msgSender());
        if(user.amount > 0) {
        ...
        if (_amount > 0 && address(crssReferral) != address(0) && _referrer != address(0) && _referrer != __msgSender()) {
            crssReferral.recordReferral(__msgSender(), _referrer);
        }
        console.log("Referral Passed");
        payOrLockuppendingCrss(_pid);
        console.log("Payor Lock");
        if (_amount > 0) {
            uint256 oldBalance = pool.lpToken.balanceOf(address(this));
            console.log("Allownace: ", pool.lpToken.balanceOf(__msgSender()), _amount, pool.lpToken.balanceOf(__msgSender()) > _amount);
            pool.lpToken.transferFrom(__msgSender(), address(this), _amount);
            console.log("Transfered");
            uint256 newBalance = pool.lpToken.balanceOf(address(this));
            _amount = newBalance.sub(oldBalance);
            console.log("Arrived: ", _amount);
            ...
            if(pool.strategy != address(0)) {
                ...
                _amount = IStrategy(pool.strategy).deposit(__msgSender(), _amount);
            }
            else if(isAuto) {
            ...
            user.isVest = isVest;
            if(user.isAuto){
                autoUserIndex(_pid, __msgSender());
            }
        }
        console.log("Passed almost");
        uint256 amount = getUserDepositBalanceByPid(_pid, __msgSender());
        user.rewardDebt = amount.mul(pool.accCrssPerShare).div(1e12);
        emit Deposit(__msgSender(), _pid, _amount);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant validatePoolByPid(_pid) {
        ...
        if (_amount > 0) {
            ...
            if(pool.strategy != address(0)) { 
                shareRemoved = IStrategy(pool.strategy).withdraw(__msgSender(), _amount);
            }
            else if(user.isAuto) {
            ...
            pool.lpToken.transfer(address(__msgSender()), _amount);
        }
        uint256 amount = getUserDepositBalanceByPid(_pid, __msgSender());
        user.rewardDebt = amount.mul(pool.accCrssPerShare).div(1e12);
        emit Withdraw(__msgSender(), _pid, _amount);
    }
    ...
    function enterStaking(uint256 _amount) external {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeCrssTransfer(__msgSender(), pending);
            }
        }
        if(_amount > 0) {
            uint256 oldBalance = pool.lpToken.balanceOf(address(this));
            pool.lpToken.transferFrom(address(__msgSender()), address(this), _amount);
            uint256 newBalance = pool.lpToken.balanceOf(address(this));
            _amount = newBalance.sub(oldBalance);
        ...
        emit Deposit(__msgSender(), 0, _amount);
    }
    ...
    function leaveStaking(uint256 _amount) external {
        ...
        if(_amount > 0) {
            ...
            pool.lpToken.safeTransfer(__msgSender(), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accCrssPerShare).div(1e12);

        emit Withdraw(__msgSender(), 0, _amount);
    }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant validatePoolByPid(_pid) {
        ...
        if(pool.strategy != address(0)) { 
            ...
            IStrategy(pool.strategy).withdraw(__msgSender(), amount);
        }
        else if(user.isAuto) {
        ...
        pool.lpToken.transfer(address(__msgSender()), amount);
        emit EmergencyWithdraw(__msgSender(), _pid, amount);
    }
        ...
        if(user.amount > 0)
        {
            ...
            if (pending > 0) {
                ...
                if(user.isVest) {
                    ...
                    safeCrssTransfer(__msgSender(), crssReward);

                    crss.approve(address(xCrss), xCrssReward);
                    xCrss.depositToken(__msgSender(), xCrssReward);
                }
                else {
                    uint256 crssReward = pending.mul(75).div(100);
                    uint256 burnReward = pending.mul(25).div(100);
                ...
                payReferralCommission(__msgSender(), pending);
            }
        }
    }
    ...
    function setDevAddress(address _devAddress) external {
        require(__msgSender() == devAddress, "setDevAddress: FORBIDDEN");
        require(_devAddress != address(0), "setDevAddress: ZERO");
        devAddress = _devAddress;
    }

    function setTreasuryAddress(address _treasuryAddress) external {
        require(__msgSender() == treasuryAddress, "setTreasuryAddress: FORBIDDEN");
        require(_treasuryAddress != address(0), "setTreasuryAddress: ZERO");
        treasuryAddress = _treasuryAddress;
    }

    // Update auto compounding fee.
    function updateAutoCompFee(uint256 _autoCompFee) external  {
    ...
    function updateEmissionRate(uint256 _crssPerBlock) external  {
        massUpdatePools();
        emit EmissionRateUpdated(__msgSender(), crssPerBlock, _crssPerBlock);
        crssPerBlock = _crssPerBlock;
    }

    // Update the CRSS referral contract address by the owner
    function setCrssReferral(ICrssReferral _crssReferral) external  {
