...
const getSupplyByBlock = async (timestamp) => {
    ...
    let request = await axios.get(`https://api.bscscan.com/api?module=stats&action=tokensupplyhistory&contractaddress=0x99FEFBC5cA74cc740395D65D384EDD52Cb3088Bb&blockno=${timestamp}&apikey=${process.env.BSC_PREMIUM_API}`)
    //delay has to exist because most BSC Pro Api requests are limited to 2/sec
    await delay(1);
    let responseData = request.data
    let responseValue = responseData.result
