...
const main = async () => {
    let txs = fs.readFileSync("./_snapshot/report/dipBuy/SellerTime.json", "utf-8")
    txs = JSON.parse(txs)
    ...
    txs = fs.readFileSync("./_snapshot/report/dipSell/SellerTime.json", "utf-8")
    txs = JSON.parse(txs)
...
const populateTime = async (txs, file) => {
    ...
    for (let i = 0; i < buyers.length; i++) {
        ...
        if (index < 0) {
            ...
            buyers[i].createdAt = txs[index].createdAt
        }
